...
async function sendNotification(text) {
  if (!BOT_TOKEN || !NOTIFY_CHAT) {
    console.warn('[notify] BOT_TOKEN или NOTIFY_CHAT_ID не заданы');
    return false;
  }
  try {
...
function formatOrderNotification(order, items) {
  ...
  const itemLines = items.map(i => {
    ...
    return `• ${i.fabric_name} (${i.color_name}) \u00D7 ${i.meters}\u00A0м — ${_fmt(i.price_per_meter)}\u00A0$/м [${type}]`;
  }).join('\n');
  ...
  return [
    ...
    itemLines,
    '',
    `\uD83D\uDCB5 Итого: ~${_fmt(totalUsd)}\u00A0$ (${totalMeters}\u00A0м)`,
...
module.exports = { sendNotification, formatOrderNotification, formatSampleNotification };
